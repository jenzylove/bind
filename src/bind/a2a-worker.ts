...
async function processTask(t: any): Promise<void> {
  ...
  // Newly assigned → apply (quote at the task's budget, escrow path).
  if (status.includes("created") && !applied.has(jobId)) {
    applied.add(jobId);
    ...
    console.log(`[a2a] applied to ${jobId} @ ${amount} USDT ->`, r?.ok ?? r?.error ?? "sent");
    return;
  }

  // Accepted (escrow funded) → run the mission for real, then deliver on-chain.
  if (status.includes("accept") && !delivered.has(jobId)) {
    delivered.add(jobId); // claim before work so a retry can't double-deliver
    const ctx = await oc(["agent", "common", "context", jobId, "--role", "asp", "--agent-id", AGENT_ID]);
    const goal = goalOf(ctx, t);
    if (!goal) { console.warn(`[a2a] ${jobId} accepted but no goal text found; skipping`); return; }

    console.log(`[a2a] ${jobId} accepted — running mission: "${goal.slice(0, 60)}"`);
    let deliverable: string;
    try {
      const plan = await createPlan({ goal });
      if (!plan.steps.length) {
        deliverable = "Bind has no marketplace agent that can genuinely deliver this task: " + (plan.note ?? "no compatible specialist found.");
      } else {
        const exec = await executePlan(plan);
        deliverable = exec.finalOutput || "No agent output passed verification for this task, so no reliable deliverable could be produced.";
      }
    } catch (e) {
    ...
    console.log(`[a2a] delivered ${jobId} ->`, r?.ok ?? r?.error ?? "sent");
  }
}
