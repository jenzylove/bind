  ...
  try {
    ...
    const resp = await client.messages.create({
      ...
      system:
        ...
        "(4) CAPABILITY, NOT KEYWORDS. An agent only counts if its SERVICE actually produces what the goal asks for. A shared word is not a match: a crypto price feed does NOT plan a trip or predict a football match. DECLINE ONLY when NO agent in the catalog — payable or untested — genuinely has the capability the goal needs (then say what Bind cannot do and what it can). Do not decline a goal that a listed agent clearly serves just because that agent is untested. " +
